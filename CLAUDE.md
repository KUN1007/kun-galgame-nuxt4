...
- Catalog **reads and user writes** go to `/v2` with `Authorization: Bearer nmk_…` (problem+json, string ids). D35d leaves two v1 surfaces permanently: `GET /api/v1/catalog/claim-events/feed` (kungal moemoepoint / stub / unpublish cron; `/v2/catalog/changes` has no from/to/actor) and S2S `POST /api/v1/catalog/works/claim` (letmoe reconcile; kungal has no caller). Name both individually in any `/api/v1/catalog/**` 410. Preview is not GA; do not 410 `/v1`.
