...
- Catalog **reads and user writes** go to `/v2` with `Authorization: Bearer nmk_…` (problem+json, string ids). Leave these on v1 until infra grows a replacement: `GET /api/v1/catalog/claim-events/feed`, `GET /api/v1/catalog/edit-revisions/feed`, `GET /api/v1/catalog/edit/proposals` (counting another user's merges), `GET /api/v1/user/catalog/edit/proposals/{id}` (others' proposals; `/v2/me/proposals/{id}` is own-only and there is no moderation GET-by-id), `POST /api/v1/user/catalog/edit/revert` (v2 wants `revision_id`, kungal still has `to_seq`), `POST /api/v1/user/catalog/edit/images`, anonymous S2S cover tallies on `GET /api/v1/catalog/works/{id}`, and claim-actions v2 does not cover (`publish` / `submit` / `ban` / `unban`). Preview is not GA; do not 410 `/v1`.
